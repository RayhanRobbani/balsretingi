...
class ManagePtcController extends Controller
{
    ...
    public function submit($request,$ptc,$isUpdate = 0)
    {
        ...
        if ( $ptc->status == 1 && $request->status == 0) {
            $ptc->status = 0;
        } elseif ($ptc->status == 2 && $request->status == 3) {
            $ptc->status = 3;
        } else {
        ...
        if($request->ads_type == 1){
            ...
            $ptc->ads_body = $request->youtube . '?autoplay=1&mute=1';
        }
    ...
    {
        $ptcAd = Ptc::where('id', $id)->first();
        $ptcAd->delete();

        return redirect()->route('admin.ptc.index')->with('success', 'PTC Ad has been deleted successfully!');
    }
}
