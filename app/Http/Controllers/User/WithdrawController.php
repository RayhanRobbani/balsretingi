...
class WithdrawController extends Controller
{
    ...
    public function withdrawMoney()
    {
        ...
        $last_ref = User::where('ref_by', auth()->id())->where('plan_id', '!=', 0)->latest()->first();
        if ($last_ref) {
            $timeNow = Carbon::now();
            $difference = date_diff($timeNow, $last_ref->created_at);
        ...
        return view($this->activeTemplate . 'user.withdraw.methods', compact('pageTitle', 'withdrawMethod', 'isAllowed'));
    }
    ...
    public function withdrawStore(Request $request)
    {
        ...
        if ($isAllowed == false) {
            $notify[] = ['error', 'You do not have referred a user in last 30 days!'];
            return back()->withNotify($notify);
        }
