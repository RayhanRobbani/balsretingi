...
class UserController extends Controller
{
    public function home()
    {
        ...
        $user = auth()->user();
        $transactions = Transaction::where('user_id', auth()->id())->orderBy('id', 'desc')->take(1)->get();
        $tran = Transaction::where('user_id', auth()->id())->orderBy('id', 'desc')->take(5)->get();

        return view($this->activeTemplate . 'user.dashboard', compact('pageTitle', 'chart', 'user', 'transactions', 'tran'));
    }
    ...
    public function buyPlan(Request $request)
    {
        ...
        if ($request->id < $user->plan_id) {
            $notify[] = ['error', 'You cannot downgrade to a subscription plan!'];
            return back()->withNotify($notify);
        }
        ...
        $ref_by = $user->ref_by;
        $subscription_plans = Referral::where('commission_type', 'plan_subscribe_commission')->limit($plan->ref_level)->get();

        foreach ($subscription_plans as $subscription_plan) {
            $amount = $plan->price * ($subscription_plan->percent / 100);
            $referral = User::where('id', $ref_by)->first();
            ...
            if (isset($referral)) {
                ...
                Transaction::create([
                    ...
                    'trx_type' => '+',
                    'trx' => $trx,
                    'details' => 'Plan Subscription Level: ' . $subscription_plan->level . ' from User: ' . $user->username,
