                                ...
                                @forelse($ads as $ptc)
                                    <tr>
                                        ...
                                        <td data-label="@lang('Status')">
                                            @if ($ptc->status == 1)
                                                <span class="text-success"><b>Acvite</b></span>
                                            @elseif ($ptc->status == 2)
                                                <span class="text-warning"><b>Pending</b></span>
                                            @elseif ($ptc->status == 3)
                                                <span class="text-danger"><b>Rejected</b></span>
                                            @else
                                                <span class="text-secondary"><b>Inactive</b></span>
                                            @endif
                                        </td>
                                        <td data-label="@lang('Action')">
                                            @if ($ptc->status == 3)
                                                <button class="btn btn--base btn-sm" disabled><i
                                                        class="la la-pen"></i></button>
                                            @else
                                                <a class="btn btn--base btn-sm"
                                                    href="{{ route('user.ptc.edit', $ptc->id) }}"><i
                                                        class="la la-pen"></i></a>
                                            @endif
                                            @if ($ptc->status == 1 || $ptc->status == 0)
                                                @if ($ptc->status == 1)
                                                    <a class="btn btn--danger btn-sm"
                                                        href="{{ route('user.ptc.status', $ptc->id) }}"><i
                                            ...
                                            @else
                                                <button class="btn btn--danger btn-sm" disabled><i
                                                        class="la la-eye-slash"></i></button>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
